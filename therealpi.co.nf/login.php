        ...
        if($results = mysqli_query($con, $query)){
                while($row = mysqli_fetch_array($results)){
                        ...
                        if(md5($entered_password) == $database_password){
                                ...
                                $welcome_query = "SELECT `welcome` FROM `users` WHERE `username`='".$entered_name."'";

                                if($results = mysqli_query($con, $welcome_query)){
                                        $_SESSION['Welcome'] = mysqli_fetch_array($results)[0];
                                }
